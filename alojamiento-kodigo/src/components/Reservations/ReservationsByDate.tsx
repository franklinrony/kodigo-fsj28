...
const ReservationsByDate: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await getReservationsByCalendar(selectedAccommodation, startDate, endDate);
      setResults(res);
      ...
      setResults([]);
      if (err.message && err.message.toLowerCase().includes('3 meses')) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <form onSubmit={handleSearch} className="space-y-6 bg-white p-6 rounded-lg shadow">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Reservación</label>
          <select
          ...
          >
            ...
            {allReservations.map(r => (
              <option key={r.id} value={r.accommodationId}>
                {`BK${r.id}-${r.guestName}`}
              </option>
            ))}
          </select>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {results.map(r => (
                  <tr key={r.id}>
                    <td className="px-4 py-2">{userCache[r.user_id]?.name || r.guestName || r.user_id || '-'}</td>
